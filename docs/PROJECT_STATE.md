...
## Current State
- Telegram 本地下载 bot 已完成 BBDown-rust Bilibili 迁移，Bilibili 下载和登录管理直接使用 `bbdown-core` crate API。当前功能包括全文 URL 扫描、微信文章自动 PDF 白名单、YouTube metadata/封面/字幕/sidecar、Bilibili 番剧/intl 下载、BBDown-rust web/tv/access-key 登录管理、外部命令进度转发、文件活性监控和超时保护，以及会迁移旧登录态、异步限流的认证探测、失败不自动降级的 Bilibili UGC 合集发现和增量合集下载。
