...
- The revised probe first runs the existing locked legacy-state migration, then uses the credential-aware `bbdown-core` client. Local loopback tests cover both a saved credential file and a legacy state file, and verify that the membership request includes its cookie.
...
- Regression tests cover saved-credential forwarding, legacy-state migration before the local mock view request, and probe-error refusal of automatic single-video download.
